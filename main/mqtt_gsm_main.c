...
static void tx_task(void *arg)
{
    ...
    while (1) {
        //sendData(TX_TASK_TAG, "AT+ATE1");
        if(!init_gps)
        {
            init_gps = true;
            sendData(TX_TASK_TAG, "AT+CGPS=1");
            vTaskDelay(4000 / portTICK_PERIOD_MS);
            taskYIELD();
        }
        else
        {
            sendData(TX_TASK_TAG, "AT+CGPSINFO");
            vTaskDelay(4000 / portTICK_PERIOD_MS);
            //printf("TX Port: %d", UART_NUM_1);
            taskYIELD();
        }
    }
}
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        ...
        case MQTT_EVENT_CONNECTED:
            ...
            esp_mqtt_client_subscribe(client, "downlink/ds/switch", 0);
            ...
            //char lat[] = "20°33\'52\".6203";
            //char separator = "\\u0000"; 
            //char *lon[] = "100°22\'04\".50396";
            //
            //char * latseparator = strcat(lat, separator);
            //char * latlon = strcat(latseparator, lon);
            //char test[] = "20°33\'52\".6203\u0000 100° 22\'04\".50396";
            //esp_mqtt_client_publish(client, "ds/location", "20°33\'52\".6203\0100°22\'04\".50396", 0, 1, 0);
            float lat = 20.33; //2033.875;
            char slat[15];
            snprintf(slat, sizeof(slat), "%f", lat);
            //esp_mqtt_client_publish(client, "ds/location", slat, 0, 1, 0);
            esp_mqtt_client_publish(client, "ds/location", "20°33\'52\".6203\n100°22\'04\".50396", 0, 1, 0);
            break;
        case MQTT_EVENT_SUBSCRIBED:
            ...
            if(!uart_comm)
            {
                xTaskCreate(rx_task, "uart_rx_task", 3072, NULL, configMAX_PRIORITIES - 2, NULL);
                xTaskCreate(tx_task, "uart_tx_task", 3072, NULL, configMAX_PRIORITIES - 3, NULL);
                uart_comm = true;
            }
...
//    }
    ...
    snprintf(uri, sizeof(uri), "mqtts://device:%s@%s:8883", "2qUNaeDHPncUyKx2JHklMOUsJvmVXzzs", CONFIG_BLYNK_SERVER);
...
static void register_auth_key_cmd(void)
{
    ...
    auth_cmd_args.token = arg_str0(NULL, NULL, "2qUNaeDHPncUyKx2JHklMOUsJvmVXzzs", "blynk auth token");
...
void app_main()
{
    ...
    mqtt_app_start();
    //xTaskCreate(mqtt_event_handler, "mqtt_event_handler", 4096, NULL, 4, NULL);
    //xTaskDelete(mqtt_app_start);
    //xTaskCreate(rx_task, "uart_rx_task", 3072, NULL, configMAX_PRIORITIES - 2, NULL);
    //xTaskCreate(tx_task, "uart_tx_task", 3072, NULL, configMAX_PRIORITIES - 3, NULL);
    //vTaskStartScheduler();
